...
for _, data in yaml_data.items():
    ...
    if entry_type=="Article":
      # for journal article, add remaining details (APA style but first names first)
    ...
    # BOOK CHAPTERS
      ...
      # for book chapter, add remaining details
      if editors is not None:
        # note hard-coded for multiple editors for now
        entry += f"In {editors} (Eds.)"
      entry += f" <em>{book}</em>"
      if pages is not None:
        entry += f", (pp. {pages})"
      ...
      # add to year list
      if year_str not in entries["chaps"]:
          entries["chaps"][year_str] = []
      entries["chaps"][year_str].append(
          "<li class='list-group-item'>" + entry + "<br>" +
          " ".join(buttons) + "</li>"
      )
...
#| id: "chapters-year"
#| output: asis
for year in sorted(entries["chaps"].keys(), reverse=True):
    display(Markdown(f"### {year}" + "{#" + f"published-{year}" + "}"))
    display(HTML(
        "<ul class='list-group list-group-flush'>" + '\n'.join(entries["chaps"][year]) + "</ul>"
    ))
```
